...
import { pngToPdf } from '@/lib/engines/imagesToPdf';

const tool = TOOL_MAP['png-to-pdf'];
...
export default function PngToPdf() {
  ...
  const handleProcess = async () => {
    ...
    setError('');
    try {
      const buf = await pngToPdf(files);
      setResult(buf);
      setStatus('success');
    } catch (e: any) {
  ...
  return (
    <ToolPage tool={tool}>
      ...
      {files.length > 0 && status === 'idle' && (
        <button type="button" onClick={handleProcess} className="btn-primary mt-6 w-full">Convert PNG to PDF</button>
      )}
      <ProcessingResult
        status={status}

...
import { wordToPdf } from '@/lib/engines/wordToPdf';

const tool = TOOL_MAP['word-to-pdf'];
...
export default function WordToPdf() {
  ...
  const handleProcess = async () => {
    ...
    setError('');
    try {
      const buf = await wordToPdf(files[0]);
      setResult(buf);
      setStatus('success');
    } catch (e: any) {
  ...
  const handleDownload = () => {
    if (result) downloadBlob(result, `${baseName(files[0].name)}.pdf`, 'application/pdf');
  };
  ...
  return (
    <ToolPage tool={tool}>
      ...
      {files.length > 0 && status === 'idle' && (
        <button type="button" onClick={handleProcess} className="btn-primary mt-6 w-full">Process Word to PDF</button>
      )}
      <ProcessingResult
        status={status}
        progressText="Converting your document..."
        error={error}
